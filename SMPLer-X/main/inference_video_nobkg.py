...
def main():
    ...
    for video_name in os.listdir(root_dir):
        ...
        if os.path.isdir(video_dir):
            for clip_name in os.listdir(video_dir):
                ...
                # if os.path.exists(os.path.join(clip_dir, "smplx","clip.pkl")):
    ...
    # print(f"共有 {len(all_videos)} 个视频需要处理, {gpu_id} 号GPU处理 {start_idx} 到 {end_idx} 个视频")


    # 单进程处理
    idx = 0
    for video_dir, clip_name, clip_dir in tqdm(all_videos, desc=f"处理GPU {gpu_id}的视频"):
        idx += 1
        filename = "clip.mp4"
        if filename.endswith('clip.mp4'):
            ...
            all_frams = []
            file_path = os.path.join(save_path, 'clip.pkl')
