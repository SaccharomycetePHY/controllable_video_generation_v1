...
def parse_args():
    ...
    parser.add_argument('--output_folder', type=str, default="video_output2", help="Output folder to save the results to.")
    ...
    parser.add_argument('--image_type', type=str, default='geometry_detail', 
        choices=["geometry_detail", "geometry_coar se", "out_im_detail", "out_im_coarse"], 
...
def process_videos_in_batch(video_list, output_dir):
    for video_path in video_list:
        # 获取视频文件的名称（不包括扩展名）
        video_name = Path(video_path).stem
        video_output_dir = os.path.join(output_dir, video_name)

        # 检查输出文件夹是否存在，存在则跳过该视频
        ...
        args.output_folder = os.path.join(output_dir, video_name)
        
        # 调用reconstruct_video处理每个视频
...
if __name__ == "__main__":
    # 设置视频文件夹路径和模型路径
    ""
    video_folder = rf"/home/ubuntu/code/emoca/data/collected_videos"  # 从上一步提取的视频存放路径
    output_dir = rf"/mnt/e/ubuntu/video_resultsimg"  # 生成的重建结果存放路径
    # path_to_models = rf"\mnt\e\path\to\your\models"  # 模型文件路径
    # model_name = "EMOCA_v2_lr_mse_20"  # 使用的模型名称

    # 获取视频列表（假设视频是.mp4格式）
    video_list = sorted([os.path.join(video_folder, f) for f in os.listdir(video_folder) if f.endswith('.mp4')])

    # 只处理前40个视频
    # video_list = video_list[6:40]
    # video_list = video_list[:5]
    

    # 批量处理视频
    process_videos_in_batch(video_list, output_dir)