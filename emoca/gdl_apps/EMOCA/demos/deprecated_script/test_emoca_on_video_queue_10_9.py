...
def reconstruct_video(args):
    ...
    for j, batch in enumerate(auto.tqdm(dl)):
        ...
        #     sample_output_folder.mkdir(parents=True, exist_ok=True)
            ...
            #     pickle.dump({k: vals[k][i].detach().cpu() for k in ['shapecode', 'texcode', 'expcode', 'posecode', 'cam', 'lightcode', 'detailcode'] if k in vals and isinstance(vals[k], torch.cuda.FloatTensor)}, f)
...
def save_codes(output_folder, name, codes):
    ...
    np.save(output_folder / name / "light.npy", codes["lightcode"])
    # 释放内存
    del codes
...
def main():
    ...
    input_videos = []
    args.input_video = '/home/zyude/human_video_data/bilibili/141248'

    args.output_folder = '/home/zyude/human_video_data/bilibili_emoca/141248'
    ...
    start_idx = (gpu_id * len(input_videos)) // num_gpus
    end_idx = ((gpu_id + 1) * len(input_videos)) // num_gpus
    
    for idx in range(start_idx, min(end_idx, len(input_videos))):
        video = input_videos[idx]
        args_copy = copy.deepcopy(args)
        args_copy.input_video = video
        args_copy.output_folder = os.path.join(args.output_folder, os.path.relpath(os.path.dirname(video), args.input_video))
        
        if os.path.exists(os.path.join(args_copy.output_folder, f"EMOCA_v2_lr_mse_20/{os.path.basename(video)}/results/video_geometry_detail.mp4")):
            print(f"已经处理过视频: {video}")
            continue
    ...
    print(f"GPU {gpu_id} 处理了从 {start_idx} 到 {min(end_idx, len(input_videos) - 1)} 号视频")
