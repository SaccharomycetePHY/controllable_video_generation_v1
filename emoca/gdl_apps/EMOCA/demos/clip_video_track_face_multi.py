    ...
    for frame_dir in tqdm(frame_dirs):
        ...
        if current_vertices:
            ...
            if frame_number == current_frame_number:
                ...
                if last_faces_count != len(current_frame_vertices_list) or int(current_clip_start) - int(last_frame_number) > 300:
                    if current_clip_start is not None:
                        clips.append((current_clip_start, last_frame_number))
                    current_clip_start = current_frame_number
                    last_vertices_dict[current_frame_number] = current_frame_vertices_list[:]
...
def get_bit_rate(video_path):
    ...
    for line in output.split('\n'):
        if 'bitrate:' in line:
            bitrate = line.split('bitrate:')[1].strip().split()[0]
            return str(int(float(bitrate) * 1000))  # 转换为bps
    return '2000'  # 如果没有找到比特率信息
...
def process_video(video_dir, video_name, output_dir):
    ...
    for T in [0.02]:
        ...
        raw_video_path = f'/zouyude/data/bilibili/{video_dir}/{video_name}.mp4'
        bit_rate =  get_bit_rate(raw_video_path)

        process_clips(annotations_file, input_frames_dir, input_emoca_dir, output_videos_dir, 30, bit_rate, sorted_sizes, sorted_centers)
...
def parse_args():
    ...
    parser.add_argument('--output_base_dir', required=True, type=str)
