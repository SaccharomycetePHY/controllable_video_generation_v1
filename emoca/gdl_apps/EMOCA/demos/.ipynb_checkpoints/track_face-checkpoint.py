...
def process_clips(annotations_file, input_frames_dir, output_videos_dir, frame_rate=30, bit_rate='200', sorted_sizes=None, sorted_centers=None):
...
def process_video(videos_list, video_name, output_dir):
    video_dir = f'/amax/data/human_videos/results/bilibili/{videos_list}/EMOCA_v2_lr_mse_20'
    results_dir = f'{video_dir}/{video_name}/results'
    
    # 检查是否已经处理过
    if os.path.exists(f'/amax/zyude/results_9_27/{videos_list}/{video_name}') or \
       os.path.exists(f'/amax/zyude/results_10_2/{videos_list}/{video_name}'):
        print(f"跳过已处理的视频: {video_name}")
        return

    print(f"正在处理: {video_dir}")  # 输出正在处理的video_dir
    ...
    for T in [0.02]:
        ...
        process_clips(annotations_file, input_frames_dir, output_videos_dir, int(fps), bit_rate, sorted_sizes, sorted_centers)
...
root_dir = '/amax/data/human_videos/results/bilibili'
    ...
    for videos_list in os.listdir(root_dir):
        ...
        if os.path.isdir(video_dir):
            ...
            for video_name in video_list:
                output_dir = f'/amax/zyude/results_10_6/{videos_list}/{video_name}/'
                futures.append(executor.submit(process_video, videos_list, video_name, output_dir))
    
    for future in concurrent.futures.as_completed(futures):
        future.result()

# process_video('562197', 'BV1BT421U7FJ', '/amax/zyude/BV11C411p72c-match-new/562197/BV1BT421U7FJ/')
