...
# 保存操作封装成异步函数
async def save_async(func, *args):
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, func, *args)


async def reconstruct_video(args):
    ...
    for j, batch in enumerate (auto.tqdm( dl)):
        ...
        for i in range(current_bs):
            ...
            vertices = vals['verts'][i].detach().cpu().numpy()
            texture = util.tensor2image(vals['uv_texture_gt'][i])
            normal_map = util.tensor2image(vals['uv_detail_normals'][i] * 0.5 + 0.5)
            # del vals['verts'][i]
            # 异步保存任务
    #         if args.save_mesh:
    #             tasks.append(save_async(save_obj, emoca, str(sample_output_folder / "mesh_coarse.obj"), vertices, texture, normal_map, i))
    #         if args.save_images:
    #             tasks.append(save_async(save_images, outfolder, name, visdict, i))
    #         if args.save_codes:
    #             tasks.append(save_async(save_codes, Path(outfolder), name, vals, i))
...
def parse_args():
    ...
    parser.add_argument('--reconstruct_video', type=str2bool, default=False, help="If true, must save images and will render the video")
...
def main():
    ...
    input_videos = []
    input_videos = ['/amax/zyude/SMPLer-X/demo/videos/BV14z421R7mc.mp4']
    ...
    for idx, video in enumerate(input_videos):
        ...
        asyncio.run(reconstruct_video(args_copy))
    
    # reconstruct_video(args)

    profiler.disable()
    # 输出性能报告
    with open("profile_report-asy-64-4.txt", "w") as f:
