...
def process_batch(batch, emoca, outfolder, args):
    ...
    max_workers = min(4, os.cpu_count() * 2)  # 适量减少线程数量，避免过度调度
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for i in range(current_bs):
            ...
            # 异步保存任务
            if args.save_mesh:
                futures.append(executor.submit(save_obj, emoca, str(sample_output_folder / "mesh_coarse.obj"), vals, i))
            if args.save_images:
                futures.append(executor.submit(save_images, outfolder, name, visdict, i))
            if args.save_codes:
                futures.append(executor.submit(save_codes, Path(outfolder), name, vals, i))
...
def reconstruct_video(args):
    ...
    for j, batch in enumerate(auto.tqdm(dl)):
        process_batch(batch, emoca, outfolder, args)
...
def main():
    ...
    profiler.disable()
    # 输出性能报告
    with open("profile_report_mul_64-4.txt", "w") as f:
