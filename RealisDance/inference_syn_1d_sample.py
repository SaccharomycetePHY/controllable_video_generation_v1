...
from src.pipelines.pipeline_fc_1d import RealisDancePipeline
...
def main(
    ...
    **kwargs,
    ...
    save_path = f"{output_dir}/vis_{output_name}.mp4"
    save_videos_grid(save_obj, save_path, fps=fps)
    # save_path = f"{output_dir}/vis/gif/{output_name}.gif"
    # save_videos_grid(save_obj, save_path, fps=fps)
    # save_videos_grid(save_obj, save_path, fps=fps)
    sample_save_path = f"{output_dir}/samples_{output_name}.mp4"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="/zouyude/storage/RealisDance/infer_output")
    parser.add_argument("--ckpt", type=str, default="/zouyude/storage/RealisDance/stage2_syn-2025-01-12T00/checkpoints/checkpoint-iter-60000.ckpt")
    ...
    parser.add_argument("--base_video_dir", type=str, default="/ssdwork/zouyude/data/data_one_person/BV1a4421Z7QA/")
    parser.add_argument("--ref_video_dir", type=str, default="/ssdwork/zouyude/data/infer_data/ref_image")
    args = parser.parse_args()

    base_dir = args.base_video_dir
    ref_dir = args.ref_video_dir

    # 遍历所有子目录
    subdirs = os.listdir(base_dir)
    for id_ in range(len(subdirs)):
        subdir = subdirs[id_]
        # base_path = os.path.join(base_dir, subdir, os.listdir(os.path.join(base_dir, subdir))[0])

        exp_config = OmegaConf.load(args.config)
        exp_config["output_dir"] = os.path.join(args.output, f"infer_sample_syn_3/{id_}")
        ...
        exp_config["ref_image_path"] = os.path.join(base_path, f"ref1.png")
        ...
        exp_config["ref_image_path"] = os.path.join(ref_dir, ref_name)
        # 使用ref_name作为保存文件名的一部分
        exp_config["save_path"] = os.path.join(exp_config["output_dir"], f"{subdir}_{ref_name.split('.')[0]}.mp4")
        ...
        try:
            ...
            print(f"处理失败: {exp_config['save_path']}, 错误信息: {e}")
            continue
