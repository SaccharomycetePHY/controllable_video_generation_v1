...
from src.models.hack_unet2d_1d import HackUNet2DConditionModel
from src.models.hack_unet3d_1d import HackUNet3DConditionModel
from src.models.reference_net_attention import ReferenceNetAttention
from src.models.orig_attention import FeedForward
from src.models.pose_guider_1d import PoseGuider, PoseGuider1D
...
class RealisDanceUnet(nn.Module):
    def __init__(
        ...
        if self.image_finetune:
            self.unet_main = HackUNet2DConditionModel.from_pretrained(pretrained_model_path, subfolder="unet", in_channels=12,
        ...
        self.pose_guider_1d = PoseGuider1D(
            image_finetune=image_finetune,
            num_conds=1,
            **pose_guider_kwargs_dict)
    ...
    def forward(
        ...
        if self.image_finetune:
            pose_emb = self.pose_guider(hamer=hamer, smpl=smpl)
            pose_emb_1d = self.pose_guider_1d(normal=normal)
        else:
            video_length = sample.shape[2]
            hamer = rearrange(hamer, "b c f h w -> (b f) c h w")
            smpl = rearrange(smpl, "b c f h w -> (b f) c h w")
            normal = rearrange(normal, "b c f h w -> (b f) c h w")
            pose_emb = self.pose_guider(hamer=hamer, smpl=smpl, video_length=video_length,
                                        temb=timestep, encoder_hidden_states=encoder_hidden_states)
            pose_emb_1d = self.pose_guider_1d(normal=normal, video_length=video_length,
                                              temb=timestep, encoder_hidden_states=encoder_hidden_states)
            # pose_emb = self.pose_guider(hamer=hamer, smpl=smpl, video_length=video_length,
            #                             temb=timestep, encoder_hidden_states=encoder_hidden_states)
