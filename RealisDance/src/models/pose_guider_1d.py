...
class PoseGuider1D(nn.Module):
    def __init__(
        ...
        super(PoseGuider1D, self).__init__()
        ...
        self.linear = nn.Sequential(
            nn.Linear(out_channels[-1] * 80, 1024),
            nn.SiLU(),
            nn.Linear(1024, 512),
            nn.SiLU())
        ...
        self.blocks_0 = nn.Sequential(
            nn.Conv2d(backbone_channels[-1]*num_conds, out_channels[0], kernel_size=(1, 1)),
            nn.SiLU()
        )
    ...
    def forward(
        ...
        last_out = outs[-1].reshape(outs[-1].shape[0], -1)
        final_out = self.linear(last_out)
        return final_out
...
class PoseGuider(nn.Module):
    ...
    def forward(
        ...
        cond = torch.cat((hamer, smpl), dim=1)
