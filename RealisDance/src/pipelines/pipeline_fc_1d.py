...
from src.models.rd_unet_1d import RealisDanceUnet
...
class RealisDancePipeline(DiffusionPipeline):
    ...
    def _encode_ground(self, bg_image, fg_image, video_length):
        with torch.no_grad():
            if self.image_finetune:
                bg_latents = self.vae.encode(bg_image).latent_dist
                bg_latents = bg_latents.mode()
                bg_latents = bg_latents * self.vae.config.scaling_factor

                fg_latents = self.vae.encode(fg_image).latent_dist
                fg_latents = fg_latents.mode()
                fg_latents = fg_latents * self.vae.config.scaling_factor
            else:
                bg_latents = self.vae.encode(rearrange(bg_image, "b c f h w -> (b f) c h w")).latent_dist
                bg_latents = bg_latents.mode()
                bg_latents = bg_latents * self.vae.config.scaling_factor
                bg_latents = rearrange(bg_latents, "(b f) c h w -> b c f h w", f=video_length)

                fg_latents = self.vae.encode(rearrange(fg_image, "b c f h w -> (b f) c h w")).latent_dist
                fg_latents = fg_latents.mode()
                fg_latents = fg_latents * self.vae.config.scaling_factor
                fg_latents = rearrange(fg_latents, "(b f) c h w -> b c f h w", f=video_length)

        return bg_latents, fg_latents
    ...
    def check_inputs(self, hamer, smpl, normal, ref_image, ref_image_clip, height, width, callback_steps):
        ...
        if not isinstance(normal, torch.Tensor):
            raise ValueError(f"`normal` has to be of type `torch.Tensor` but is {type(normal)}")
    ...
    def __call__(
        ...
        **kwargs,
        ...
        self.check_inputs(hamer, smpl, normal, ref_image, ref_image_clip, height, width, callback_steps)

        # Define call parameters
        batch_size = ref_image_clip.shape[0]
        if not self.image_finetune:
            video_length = hamer.shape[2]
        else:
            video_length = -1
        ...
        # Encode background and foreground
        bg_latents, fg_latents = self._encode_ground(bg_image, fg_image, video_length)
        ...
        num_channels_latents = self.unet.in_channels // 3
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat((latent_model_input, bg_latents, fg_latents), dim=1)
                if self.image_finetune:
                    noise_pred = self.unet(
                        sample=latent_model_input, ref_sample=ref_latents,
                        # pose=pose, 
                        hamer=hamer, smpl=smpl, normal=normal, timestep=t,
                    ...
                    counter = torch.zeros_like(latent_model_input[:, :num_channels_latents]).to(device)
                    noise_pred = torch.zeros_like(latent_model_input[:, :num_channels_latents]).to(device)
                    if guidance_scale > 1.0:
                        noise_uncond = torch.zeros_like(latent_model_input[:, :num_channels_latents]).to(device)
                    for c in context_queue:
                        partial_latent_model_input = latent_model_input[:, :, c]
                        # partial_pose = pose[:, :, c]
                        partial_hamer = hamer[:, :, c]
                        partial_smpl = smpl[:, :, c]
                        partial_normal = normal[:, :, c]
                        # predict the noise residual
                        noise_pred[:, :, c] += self.unet(
                            sample=partial_latent_model_input, ref_sample=ref_latents,
                            # pose=partial_pose, 
                            hamer=partial_hamer, smpl=partial_smpl, normal=partial_normal, timestep=t,
                            encoder_hidden_states=clip_latents, drop_reference=False,
                        ).sample.to(dtype=latents_dtype)
                        counter[:, :, c] += 1
        ...
        # Post-processing
        video = self.decode_latents(latents, ref_image, False)
