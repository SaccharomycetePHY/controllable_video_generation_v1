...
class VideoDataset(Dataset):
    ...
    def get_batch(self, data_id):
        video_path = os.path.join(self.load_dir, data_id, 'human.mp4')
        ...
        assert len(video_reader) == len(hamer_reader) == len(smpl_reader)
        ...
        if self.image_finetune:
            ...
            ref_hamer, ref_smpl = hamer[0], smpl[0]
            hamer, smpl = hamer[1], smpl[1]
            # bg, fg = bg[1], fg[1]
        else:
            ref_image, image = ref_image[0], image[1:]
            ref_hamer, ref_smpl = hamer[0], smpl[0]
        ...
        return image, hamer, smpl, ref_image, ref_hamer, ref_smpl # , bg, fg
    ...
    def __getitem__(self, idx):
        ...
        while True:
            try:
                ...
                image, hamer, smpl, _ref_image, ref_hamer, ref_smpl = self.get_batch(data_id)
                ...
                if self.image_finetune:
                    return {"data_key": data_id, "image": image,
                            # "pose": pose,
                            "hamer": hamer, "smpl": smpl,
                            "ref_image": ref_image, "ref_image_clip": ref_image_clip,
                            # "ref_pose": ref_pose,
                            "ref_hamer": ref_hamer, "ref_smpl": ref_smpl}
