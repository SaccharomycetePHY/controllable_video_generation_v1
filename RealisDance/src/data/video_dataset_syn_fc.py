...
class VideoDataset(Dataset):
    def __init__(
        ...
        self.normal_transform = transforms.Compose([
            # ratio is w/h
            transforms.RandomResizedCrop(
                sample_size, scale=(0.75, 1),  # 随机缩放范围在0.8-1.2之间
    ...
    def get_batch(self, data_id):
        ...
        #         pad_w = (target_w - w) // 2
        #         if len(img.shape) == 3:
        #             padded = torch.ones(3, h, target_w)
        #             # print(padded.shape, img.shape, is_syn)
        #             padded[:, :, pad_w:pad_w+w] = img
        #             return F.interpolate(padded.unsqueeze(0), size=(w, h), mode='bilinear', align_corners=False).squeeze(0)
        #         else:
        #             padded = torch.ones(img.shape[0], 3, h, target_w)
        #             padded[:, :, :, pad_w:pad_w+w] = img
        #             return F.interpolate(padded, size=(w, h), mode='bilinear', align_corners=False)
            
        #     # 对所有图片进行处理
        #     image = pad_and_resize(image)
        #     hamer = pad_and_resize(hamer) 
        #     smpl = pad_and_resize(smpl)
        #     ref_image = pad_and_resize(ref_image)
        #     ref_hamer = pad_and_resize(ref_hamer)
        #     ref_smpl = pad_and_resize(ref_smpl)
        #     normal = pad_and_resize(normal)
        #     ref_normal = pad_and_resize(ref_normal)
        #     bg = pad_and_resize(bg)
        #     fg = pad_and_resize(fg)
