...
from src.utils.util_syn_fc import get_distributed_dataloader, sanity_check
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if unet_checkpoint_path != "":
        if is_main_process:
            logging.info(f"Loading checkpoint from: {unet_checkpoint_path}")
        ...
        for k, v in state_dict.items():
            ...
            # 如果是pose_attn相关的权重，需要特殊处理
            if 'pose_attn' in new_k:
                new_k = new_k.replace('2d', '3d')  # 适配3D模型
            new_state_dict[new_k] = v
        
        # 加载权重
        missing_keys, unexpected_keys = unet.load_state_dict(new_state_dict, strict=False)
        if is_main_process:
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            normal = batch["normal"].to(local_rank)
            ref_image = batch["ref_image"].to(local_rank)
            ref_image_clip = batch["ref_image_clip"].to(local_rank)

            drop_condition_ratio = 0.01
            if random.random() < drop_condition_ratio:
                hamer = torch.zeros_like(hamer)
            if random.random() < drop_condition_ratio:
                smpl = torch.zeros_like(smpl)
            if random.random() < drop_condition_ratio:
                normal = torch.zeros_like(normal)

            bg_image = batch["bg_image"].to(local_rank)
            fg_image = batch["fg_image"].to(local_rank)
            ...
            if not image_finetune and pose_shuffle_ratio > 0:
                ...
                if random.random() < pose_shuffle_ratio:
                    rand_idx = torch.randperm(L).long()
                    normal[:, :, rand_idx[0], :, :] = normal[:, :, rand_idx[rand_idx[0]], :, :]
            ...
            with torch.no_grad():
                if not image_finetune:
                    ...
                    bg_image = rearrange(bg_image, "b c f h w -> (b f) c h w")
                    fg_image = rearrange(fg_image, "b c f h w -> (b f) c h w")
                ...
                bg_latents = vae.encode(bg_image).latent_dist
                bg_latents = bg_latents.sample()
                bg_latents = bg_latents * vae.config.scaling_factor

                fg_latents = vae.encode(fg_image).latent_dist
                fg_latents = fg_latents.sample()
                fg_latents = fg_latents * vae.config.scaling_factor
            ...
            noisy_latents = torch.cat((noisy_latents, bg_latents, fg_latents), dim=1)
        ...
        if is_main_process:
            writer.add_scalar("epoch_loss", epoch_loss / len(train_dataloader), epoch)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="/storage/zouyude/RealisDance")
    ...
    profiler.disable()
    # 输出性能报告
    with open("profile_report_hamer8.txt", "w") as f:
