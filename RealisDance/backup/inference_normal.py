...
def simple_reader(ref_image_path, hamer_path, smpl_path, normal_path, bg_path, fg_path, sample_size, clip_size, max_length):
    scale = (1.0, 1.0)
    ...
    normal_transform = transforms.Compose([
        # ratio is w/h
        transforms.RandomResizedCrop(
            sample_size, scale=(0.8, 1.0),  # 随机缩放范围在0.8-1.2之间
            ratio=(0.75 * sample_size[1]/sample_size[0], 1.25 * sample_size[1]/sample_size[0]),  # 允许长宽比在原始比例的0.75-1.25倍之间变化
            antialias=True),
    ])
...
def main(
    ...
    **kwargs,
    ...
    val_ref_image, val_ref_image_clip, val_hamer, val_smpl, val_normal, val_bg_image, val_fg_image = simple_reader(
        ref_image_path=ref_image_path,
        hamer_path=hamer_path,
        smpl_path=smpl_path,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt", type=str, default="outputs/stage1_normal-2024-11-23T01/checkpoints/checkpoint-iter-86000.ckpt")
    parser.add_argument("--ref", type=str, default="/zouyude/zyd.jpg")
    parser.add_argument("--smpl", type=str, default="smpl.mp4")
    parser.add_argument("--normal", type=str, default="normal_cover.mp4")
    parser.add_argument("--hamer", type=str, default="emoca.mp4")
    parser.add_argument("--bg", type=str, default="inpaint_out.mp4")
    parser.add_argument("--fg", type=str, default="object.mp4")
    ...
    parser.add_argument("--cH", type=int, default=640)
    parser.add_argument("--cW", type=int, default=480)
    parser.add_argument("--max-L", type=int, default=80)
    args = parser.parse_args()

    base_path = os.path.join("/zouyude/data/data_one_person/eval/BV1VS411w77w/clip_7378-7516")

    exp_config = OmegaConf.load(args.config)
    exp_config["output_dir"] = os.path.join(args.output, "normal_mask", "BV1VS411w77w")
    exp_config["unet_checkpoint_path"] = args.ckpt
    exp_config["ref_image_path"] = args.ref
    exp_config["smpl_path"] = os.path.join(base_path, args.smpl)
    exp_config["normal_path"] = os.path.join(base_path, args.normal)
    exp_config["hamer_path"] = os.path.join(base_path, args.hamer)
    exp_config["bg_path"] = os.path.join(base_path, args.bg)
    exp_config["fg_path"] = os.path.join(base_path, args.fg)
