...
from src.utils.util_mask import get_distributed_dataloader, save_videos_grid, sanity_check
...
def calculate_metrics(gt, pred, lpips_model):
    gt = gt.squeeze(0)
    pred = pred.squeeze(0)
    ...
    **kwargs,
    ...
    for val_batch in tqdm(validation_dataloader):
        ...
        for idx, data_id in enumerate(val_batch["data_key"]):
            ...
            if image_finetune:
                psnr, ssim, lpips = calculate_metrics((val_gts.cpu() / 2 + 0.5).clamp(0, 1), samples.cpu(), lpips_model)
                print(f"{data_id}: PSNR: {psnr}, SSIM: {ssim}, LPIPS: {lpips}")
                psnr_list.append(psnr)
                ssim_list.append(ssim)
                lpips_list.append(lpips)
    import numpy as np
    print(f"PSNR: {np.mean(psnr_list)}, SSIM: {np.mean(ssim_list)}, LPIPS: {np.mean(lpips_list)}")
    with open(f"{output_dir}/metrics_copy.txt", "a") as f:
        f.write(f"PSNR: {np.mean(psnr_list)}, SSIM: {np.mean(ssim_list)}, LPIPS: {np.mean(lpips_list)}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="configs/stage1_normal.yaml")
    parser.add_argument("--output", type=str, default="eval_outputs")
    parser.add_argument("--ckpt", type=str, default="outputs/stage1_normal-2024-11-22T12/checkpoints/checkpoint-iter-90000.ckpt")
    ...
    exp_config["output_dir"] = os.path.join(args.output, "stage1_normal_mask/90000/eval",str(exp_config["global_seed"]))

    for i in range(1, 9):
        exp_config["unet_checkpoint_path"] = f"outputs/stage1_normal-2024-11-23T01/checkpoints/checkpoint-iter-{i}0000.ckpt"
        exp_config["output_dir"] = os.path.join(args.output, f"stage1_normal_mask/{i}0000/eval",str(exp_config["global_seed"]))
