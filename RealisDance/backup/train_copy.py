...
from src.utils.util import get_dataloader, sanity_check
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if epoch == first_epoch and step == 0:
                sanity_check(batch, f"{output_dir}/sanity_check", image_finetune, 0)

            """ >>>> Training >>>> """
            # Get images
            image = batch["image"].to(device)
            # pose = batch["pose"].to(device)
            hamer = batch["hamer"].to(device)
            smpl = batch["smpl"].to(device)
            ref_image = batch["ref_image"].to(device)
            ref_image_clip = batch["ref_image_clip"].to(device)

            bg_image = batch["bg_image"].to(device)
            fg_image = batch["fg_image"].to(device)
            ...
            # Save checkpoint
            if global_step % checkpointing_steps == 0 or global_step in checkpointing_steps_tuple:
