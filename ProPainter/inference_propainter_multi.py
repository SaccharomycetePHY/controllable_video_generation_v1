...
def read_mask(human_mask_path, object_mask_path, length, size, flow_mask_dilates=8, mask_dilates=5):
    masks_img = []
    masks_dilated = []
    flow_masks = []
    
    human_masks = np.load(human_mask_path)['masks']
    object_masks = np.load(object_mask_path)['masks']
    
    for i in range(length):
        mask = human_masks[i] | (object_masks[i] > 0)
        mask = mask.squeeze()
        mask_img = (mask * 255).astype(np.uint8)
        if size is not None:
            mask_img = Image.fromarray(mask_img).resize(size, Image.NEAREST)
...
def process_video(args, video_name, clip_name, clip_dir, device):
    if os.path.exists(os.path.join(clip_dir, 'frames')):
        frames_dir = os.path.join(clip_dir, 'frames')
    else:
        frames_dir = os.path.join(clip_dir, 'clip.mp4')
    human_mask_path = os.path.join(clip_dir, 'human_masks.npz')
    object_mask_path = os.path.join(clip_dir, 'object_masks.npz')
    
    frames, fps, size, _ = read_frame_from_videos(frames_dir)
    ...
    if args.mode == 'video_inpainting':
        ...
        flow_masks, masks_dilated = read_mask(human_mask_path, object_mask_path, frames_len, size, 
    ...
    print(f'\nProcessing: {clip_name} [{video_length} frames]...')
    with torch.no_grad():
        # ---- compute flow ----
...
def process_videos(gpu_id, gpu_videos, args):
    device = f"cuda:{gpu_id}"
    torch.cuda.set_device(device)

    for video_dir, clip_name, clip_dir in tqdm(gpu_videos, desc=f"处理GPU {gpu_id}的视频"):
        if os.path.exists(os.path.join(clip_dir, 'inpaint_out.mp4')):
            print(f"视频 {clip_dir} 已处理")
            continue
        
        try:
            process_video(args, video_dir, clip_name, clip_dir, device)
        except Exception as e:
            print(f"处理视频 {clip_dir} 时发生错误: {str(e)}")
            continue
...
if __name__ == '__main__':
    ...
    print(f"处理的视频数量: {len(all_videos)}")
    for gpu_id in range(num_gpus):
        ...
        p = mp.Process(target=process_videos, args=(gpu_id, gpu_videos, args))
        p.start()
        processes.append(p)
